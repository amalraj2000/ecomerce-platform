...
<template>
    <Head title="My Transactions - Vendor Panel" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                Vendor Transactions &amp; Sales
            </h2>
        </template>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-3xl font-extrabold text-emerald-600 mt-1">${{ Number(stats.total_sales).toFixed(2) }}</h3>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center justify-between">
                        <div>
                            ...
                            <h3 class="text-3xl font-extrabold text-indigo-600 mt-1">{{ stats.transaction_count }}</h3>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                        <div class="flex items-center space-x-3 w-full sm:w-auto">
                            <input
                                ...
                                type="text"
                                placeholder="Search order ID, intent ID..."
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-gray-50/50">
                                    ...
                                    <th class="py-3.5 px-4">Stripe Reference</th>
                            ...
                            <tbody class="divide-y divide-gray-100 text-sm">
                                <tr v-for="t in transactions.data" :key="t.id" class="hover:bg-gray-50/60 transition">
                                    <td class="py-4 px-4 font-medium text-gray-900">
                                        <Link :href="route('vendor.orders.show', t.id)" class="text-indigo-600 hover:underline">
                                            #{{ t.id }}
                                        </Link>
                                    ...
                                    <td class="py-4 px-4 text-right">
                                        <Link
                                            :href="route('vendor.orders.show', t.id)"
